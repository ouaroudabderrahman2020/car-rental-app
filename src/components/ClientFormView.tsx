...
export default function ClientFormView({ initialData, reservations = [], onRefresh, onSuccess }: ClientFormViewProps) {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      setNationalId(initialData.national_id || initialData.id_card_number || '');
      setDob(initialData.dob || '');
      setNationality(initialData.nationality || '');
      setLicenseNumber(initialData.license_number || '');
      setLicenseExpiry(initialData.license_expiry || '');
      setLicenseIssue(initialData.license_issue || '');
      setPhone(initialData.phone || '');
      setEmail(initialData.email || '');
      setAddress(initialData.address || '');
      setTrustRank(initialData.trust_rank?.toString() || '0');
      setNotes(initialData.notes || '');
      setIsBlacklisted(initialData.is_blacklisted || false);
      setIdDocUrl(initialData.drive_id_photo || '');
      setLicenseDocUrl(initialData.drive_license_front_photo || '');
      setMasterDocUrl(initialData.drive_contract_doc_id || '');
      setIdDocFile(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (initialData) {
        oldFolderName = `${initialData.name} ${initialData.national_id || initialData.id_card_number || ''}`.trim();
  ...
  const handleDelete = async () => {
    ...
    if (confirmed) {
      ...
      setStatus(t('common.processing', 'Processing...'), 'processing', 0);
      try {
        const folderName = `${initialData.name} ${initialData.national_id || initialData.id_card_number || ''}`.trim();
  ...
  const clientReservations = initialData ? reservations
    .filter(r => r.customer_phone === initialData.phone)
  ...
  return (
    <div className="bg-slate-50/50 w-full py-4 sm:py-10">
      <div className="flex items-center justify-between w-full px-4 sm:px-10 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-black flex items-center gap-3">
              {!initialData ? t('clientForm.addTitle', 'Add New Client') : t('crm.modal.title', 'Client Profile')}
