...
const FileDropZone = ({ 
  ...
  return (
    <div className="flex flex-col">
      ...
      <div 
        onClick={() => {
          if (isEditMode && !isSubmitting && !url) {
        ...
        className={`w-full aspect-video border border-black rounded-[12px] bg-white flex flex-col items-center justify-center transition-all overflow-hidden bg-clip-padding relative group ${isEditMode && !isSubmitting ? 'cursor-pointer hover:ring-4 hover:ring-slate-100' : url ? 'cursor-pointer hover:bg-slate-50' : 'cursor-default'}`}
      >
        {(file || url) ? (
          <>
            {isPdf || (file && file.contentType === 'application/pdf') ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="mt-1 text-[8px] font-black uppercase text-blue-600 group-hover:underline">
                  {isEditMode ? 'CHANGE PDF' : 'VIEW PDF'}
                </div>
              </div>
            ) : (
              <img 
      ...
      {(file || url) && isEditMode && !isSubmitting && (
        <button 
          onClick={(e) => { e.stopPropagation(); onRemove(); }}
    ...
    if (isOpen) {
      ...
      if (mode === 'edit' && carData) {
        console.log('--- DEBUG: LOADING CAR FOR EDIT ---', {
          ...
          docs: carData.documentation_url,
          reg: carData.registration_card_url,
          ins: carData.insurance_url,
          vig: carData.vignette_url
        });
        ...
        setVignetteExpiry(carData.vignette_expiry || '');
        setFirstUseDate(carData.first_use_date || '');
        
        // Ensure these URLs are correctly mapped to state
        setImageUrl(carData.image_url || '');
        setDocUrl(carData.documentation_url || '');
        setRegCardUrl(carData.registration_card_url || '');
        setInsuranceUrl(carData.insurance_url || '');
        setVignetteUrl(carData.vignette_url || '');

        setEssentials(carData.essentials?.filter(e => e.checked).map(e => e.name) || carData.essentials?.map(e => e.name) || []);
        
        const mTable: Record<string, { value: string, lastCompleted: string }> = {};
        serviceOptions.forEach(opt => {
          mTable[opt.value] = { value: '', lastCompleted: '' };
        });
        (carData.intervals || []).forEach(interval => {
      ...
      <div className="bg-slate-50/30 w-full py-4 sm:py-10">
        ...
        <div className="px-4 sm:px-10 space-y-8">
          ...
          <ModalSection1 title={t('carForm.documents')}>
            ...
            {/* Row 1: Image + 3-Col Docs */}
              <div className="flex flex-col lg:flex-row gap-4">
                {/* Vehicle Image Upload (Left) */}
              ...
              {/* Row 2: Car Documentation PDF (Bottom Anchor) */}
                  ...
                  <div className="flex flex-col gap-4 items-start w-full min-w-0">
                    <div className="flex flex-col gap-3 items-start w-full min-w-0">
                      ...
                      {(docFile || docUrl) && (
                        <div 
                          onClick={() => {
                            if (docUrl && !isEditMode) {
                              window.open(getDrivePreviewUrl(docUrl), '_blank');
                            }
                          }}
                          className={`flex-1 flex items-center justify-between px-4 h-11 border-2 rounded-[12px] overflow-hidden min-w-0 w-full transition-all 
                            ${!isEditMode && docUrl ? 'cursor-pointer hover:bg-blue-100 active:scale-[0.98]' : ''} 
                            ${docUrl ? 'bg-blue-50 border-blue-600/20' : 'bg-slate-50 border-black/10'}`}
                        >
                          <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="px-6 py-10 sm:px-10 bg-slate-50 border-t border-black/10 flex flex-col sm:flex-row justify-end items-center gap-3">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button 
              disabled={isSubmitting}
              onClick={isEditMode ? handleConfirm : () => setIsEditMode(true)}
            ...
            >
              ...
              {isSubmitting ? t('carForm.processing') : (
                isEditMode 
                  ? (mode === 'add' ? t('carForm.confirm') : t('carDetails.lockSave'))
                  : t('carDetails.editProfile')
              )}
            </button>
          </div>
        </div>
