...
import { Search, FileText, Upload, CreditCard, Monitor, X, ChevronDown, CheckCircle, Sparkles, Loader2, AlertCircle, RotateCcw, Car as CarIcon, ChevronRight, Check, Archive, Trash2, ExternalLink, Copy } from 'lucide-react';
...
import CarSelectModal from './carSelect';
...
export default function ResForm({ reservation, onChange, onSaved, mode = 'add', editId = null, onActionsReady, onSavingChange }: ResFormProps) {
  ...
  useEffect(() => {
    supabase.from('cars').select('id, brand, model, plate, status, daily_rate, odometer, essentials, documents').neq('status', 'Decommissioned').then(({ data: cars }) => {
      if (cars) {
  ...
  return (
    ...
    <div className="p-6 max-h-[calc(100vh-180px)] overflow-y-auto black-scrollbar">
      ...
      <CarSelectModal
        ...
        cars={availableCars}
        selectedCarId={reservation?.selectedCarId || null}
